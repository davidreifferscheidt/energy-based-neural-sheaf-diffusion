...
class OBGNDataset(InMemoryDataset):
    """Adapted from https://github.com/snap-stanford/ogb/blob/745531be13c5403a93c80e21a41848e38ea7637c/ogb/nodeproppred/dataset_pyg.py"""

    def __init__(
        self, name, root="dataset", transform=None, pre_transform=None, meta_dict=None
    ):
        """
        ...
        if osp.isdir(self.root) and (
            not osp.exists(
                osp.join(
                    self.root, "RELEASE_v" + str(self.meta_info["version"]) + ".txt"
                )
            )
        ):
        ...
        self.download_name = self.meta_info[
            "download_name"
    ...
    def get_idx_split(self, split_type=None):
        ...
        if self.is_hetero:
            (
                ...
                valid_idx_dict,
                test_idx_dict,
            ) = read_nodesplitidx_split_hetero(path)
            for nodetype in train_idx_dict.keys():
                ...
                test_idx_dict[nodetype] = torch.from_numpy(test_idx_dict[nodetype]).to(
                    torch.long
                )
