...
def get_logger():
    ...
    logger_name = module.__name__.split(".")[-1] if module is not None else None
    return logging.getLogger(logger_name)


@rank_zero_only
def print_config(config: DictConfig, theme: str = "default") -> None:
    content = OmegaConf.to_yaml(config, resolve=True)
    rich.print(Syntax(content, "yaml", theme=theme))
